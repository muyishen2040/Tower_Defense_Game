...
const int PlayScene::SpawnGridPointy = -1;
const int PlayScene::EndGridPointy = 7;
...
void PlayScene::Initialize() {
	...
	columnNum = std::vector<int>(MapWidth);
	...
	AddNewObject(MoneyGroup = new Group());
...
void PlayScene::Update(float deltaTime) {
	...
	for (int i = 0; i < SpeedMult; i++) {
        ...
        for(int j = 0 ; j < std::get<2>(current) ; j++){
            ...
            switch (std::get<0>(current)) {
				...
				case 3:
					...
					break;
                case 4:
					EnemyGroup->AddNewObject(enemy = new BigHeadEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
					break;
                case 5:
					EnemyGroup->AddNewObject(enemy = new GhostEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
					break;
        ...
        if (!moneyWaveData.empty()) {
            auto current2 = moneyWaveData.front();
            //if (ticks < std::get<1>(current2))//change
                //continue;
            //ticks -= std::get<1>(current2);
            moneyWaveData.pop_front();
            std::random_device dev2;
            std::mt19937 rng2(dev2());
            std::shuffle(columnNum.begin(), columnNum.end(), rng2);
            std::uniform_int_distribution<std::mt19937::result_type> dist2(1, 3);
            PaperMoney* papMon;
            for(int j = 0 ; j < std::get<2>(current2) ; j++){//SpawnGridPointy * BlockSize + BlockSize / 2 + dist2(rng2) * 15
            const Engine::Point SpawnCoordinate = Engine::Point(columnNum[j] * BlockSize + BlockSize / 2 , 0);
            switch (std::get<0>(current2)) {
				case 1:
                    //std::cout<<"mcheck11\n";
					MoneyGroup->AddNewObject(papMon = new Money500(SpawnCoordinate.x, SpawnCoordinate.y));
					break;
				case 2:
					MoneyGroup->AddNewObject(papMon = new Money1000(SpawnCoordinate.x, SpawnCoordinate.y));
					break;
				default:
					continue;
			}
...
void PlayScene::OnMouseDown(int button, int mx, int my) {
	...
	if(button){
        std::list<IObject*> l1 = MoneyGroup->GetObjects();
        std::list<IObject*>::iterator itr;
        for(itr = l1.begin();itr!=l1.end();itr++){
            if(Engine::Collider::IsPointInRect(Engine::Point(mx,my),(*itr)->Position-Engine::Point(40,20),Engine::Point(80,40))){
                if(dynamic_cast<PaperMoney*>(*itr)->money==500){
                    EarnMoney(500);
                }
                else{
                    EarnMoney(1000);
                }
                MoneyGroup->RemoveObject((*itr)->GetObjectIterator());
                break;
            }
        }
	}
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	const int y = my / BlockSize;
	if (button & 1) {
		if(preview!=nullptr&&preview->GetPrice()==0){
            //std::cout<<"check\n";
            if(mapState[y][x] == TILE_OCCUPIED){
                std::list<IObject*>::iterator it=TowerGroup->GetObjects().begin();
                for(; it!=TowerGroup->GetObjects().end();it++){
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode==ALLEGRO_KEY_TAB) {
        ...
        // TODO 4 (3/3): Check whether the input sequence is correct
            if(keyStrokes.size()==code.size()){
                bool check=true;
                std::list<int>::iterator itr=keyStrokes.begin();
                for(int i=0;itr!=keyStrokes.end();itr++,i++){
                    //std::cout<<i<<": "<<"*itr"<<*itr<<" "<<code[i]<<"\n";
                    if(*itr!=code[i]){
                    //std::cout<<"*itr"<<*itr<<" "<<code[i]<<"\n";
                        check=false;
                        break;
                    }
                }
...
void PlayScene::AddLives(int lives) {
    this->lives += lives;
...
void PlayScene::ReadMoneyWave(){
    std::string filename = std::string("resources/money") + std::to_string(MapId) + ".txt";
	// Read enemy file.
	float type, wait, lane,repeat; //change
	moneyWaveData.clear();
	std::ifstream fin(filename);
	while (fin >> type && fin >> wait && fin >> lane && fin >> repeat) {
		for (int i = 0; i < repeat; i++)
			moneyWaveData.emplace_back(type, wait, lane);
	}
	fin.close();
}
void PlayScene::ConstructUI() {
    ...
    btn = new TurretButton("play/floor.png", "play/dirt.png",
		...
		, 410, 128*MapHeight, PotatoTurret::Price);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 2));
	UIGroup->AddNewControlObject(btn);
    //
    btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/turret-4.png", 540, BlockSize*MapHeight, 0, 0, 0, 0)
		, 530, 128*MapHeight, ExplosiveTurret::Price);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
	UIGroup->AddNewControlObject(btn);
	//
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/turret-5.png", 660, BlockSize*MapHeight, 0, 0, 0, 0)
		, 650, 128*MapHeight, PoisonTurret::Price);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 4));
	UIGroup->AddNewControlObject(btn);
    //Shovel
    btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/bomb.png", 800, BlockSize*MapHeight, 0, 0, 0, 0)
		, 790, 128*MapHeight, Shovel::Price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	else if (id == 2 && money >= PotatoTurret::Price)
		preview = new PotatoTurret(0, 0);
    else if (id == 3 && money >= ExplosiveTurret::Price)
		preview = new ExplosiveTurret(0, 0);
    else if (id == 4 && money >= PoisonTurret::Price)
		preview = new PoisonTurret(0, 0);
